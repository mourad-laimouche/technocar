...
import { WHATSAPP_NUMBERS, createWhatsAppContactLink } from "@/lib/whatsapp";
...
export default function WhatsAppContactSelector({ message = "Bonjour, je souhaite obtenir des informations sur vos services de location", className }: WhatsAppContactSelectorProps) {
  const [isOpen, setIsOpen] = useState(false);

  const handleNumberSelect = (phoneNumber: string) => {
    const whatsappUrl = createWhatsAppContactLink(message, phoneNumber);
        ...
        <div className="space-y-3 pt-4">
          ...
          <div className="pt-4 border-t">
            <p className="text-xs text-center text-gray-500">
              Vous serez redirigé vers WhatsApp avec votre message
            </p>
          </div>
        </div>
      </DialogContent>
